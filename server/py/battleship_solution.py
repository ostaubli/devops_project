    ...
    def apply_action(self, action: BattleshipAction) -> None:
        if action.action_type == 'set_ship':
            ...
            opponent_ship_locations = [
                loc for ship in self.get_player_ships(False) if ship.location is not None 
...
class Battleship(Game):
    ...
    def get_shoot_actions(self) -> List[BattleshipAction]:
        loc_options = set(self.shoot_locations).difference(
            set(self.state.get_player_shots(active_player=True))
        )
        return [
            BattleshipAction(action_type=ActionType.SHOOT, location=[loc])
...
if __name__ == "__main__":
    game = Battleship()
    game.get_state()
    debug1 = game.get_state()
    print("debug 1\n","*"*50, debug1)
    game.get_list_action()
    debug2 = game.get_list_action()
    print("debug 2\n","*"*50,debug2)
    game.print_state()
    debug3 = game.print_state()
    print("debug 3\n","*"*50,debug3)
